...
public class RaidLootEditWindow extends Window
{
    ...
    public RaidLootEditWindow(Raid raid, RaidItem item)
    {
        ...
        this.setCaption("Edit loot: " + item.getName() + " for raid : " + raid.
                getComment());
    ...
    {
        price.setValue(getItemPrice(itemname.getValue().toString(), heroic.
                booleanValue()));
    }
    ...
    {
        int lootid = ItemDB.getLootId(item.getId(), CharDB.getCharacterId(item.
                getLooter()), item.getPrice(), item.isHeroic(), raid.getId());

        RaidDB.doUpdateLoot(lootid, looter, itemname, price, heroic, raid.
                getId());
    }
