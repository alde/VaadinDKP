...
public class CharacterEditWindow extends Window implements CharacterInfoListener
{
    ...
    private Button characterEditUpdateButton(final TextField name, final ComboBox characterClass, final CheckBox active)
    {
        ...
        updateButton.
                addListener(new updateBtnClickListener(name, characterClass, active));
        return updateButton;
    }
    ...
    {
        CharDB.
                removeLootFromCharacter(item.getItemProperty("Name").toString(), user);
    }

    private void updateLootForCharacter(Item item, int lootid)
    {
        CharDB.
                updateLootForCharacter(item.getItemProperty("Name").toString(), Double.
                parseDouble(item.getItemProperty("Price").toString()), Boolean.
                parseBoolean(item.getItemProperty("Heroic").toString()), user, lootid);
    }
    ...
    private class updateBtnClickListener implements ClickListener
    {
        ...
        {
            updateCharacter(name.getValue().toString(), characterClass.
                    getValue().toString(), (Boolean) active.getValue());
            notifyListeners();
            close();
        }
    }
    ...
    private void raidsAttended()
    {
        ...
        attended30.
                setValue("Attended " + att30 + "% of raids the last 30 days, ");
    ...
    private void doUpdateLoots()
    {
        for (Iterator i = loots.getItemIds().iterator(); i.hasNext();) {
            ...
            if (item.getItemProperty("Delete").toString().
