...
public class ItemDB
{
    ...
    public static List<Items> getItems()
    {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Items tempitem = new Items(rs.getInt("id"), rs.getString("name"), rs.
                        getInt("wowid_normal"), rs.getDouble("price_normal"), rs.
                        getInt("wowid_heroic"), rs.getDouble("price_heroic"), rs.
                        getString("slot"), tempType, rs.getInt("ilvl"), rs.
                        getString("quality"));
    ...
    private static Collection<ItemLooter> getLootersFormItems(int itemId)
    {
        ...
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("SELECT * FROM loots JOIN characters JOIN items JOIN raids WHERE loots.character_id=characters.id AND loots.item_id=? AND items.id=loots.item_id AND raids.id=loots.raid_id");
            p.setInt(1, itemId);
            ResultSet rs = p.executeQuery();
            while (rs.next()) {
    ...
    public static List<ItemPrices> getDefaultPrices()
    {
        ...
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("SELECT * FROM default_prices");
            ResultSet rs = p.executeQuery();
            while (rs.next()) {
                Slots slot = Slots.valueOf(rs.getString("slot"));
                prices.
                        add(new ItemPrices(slot, rs.getDouble("price_normal"), rs.
                        getDouble("price_heroic")));
            }
        } catch (SQLException e) {
        }
        return prices;
    ...
    public static List<Multiplier> getMultipliers()
    {
        ...
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("SELECT * FROM multiplier");
            ResultSet rs = p.executeQuery();
            while (rs.next()) {
                multi.add(new Multiplier(rs.getInt("id"), rs.getInt("ilvl"), rs.
    ...
    public static void addMultiplier(int ilvl, double multiplier)
    {
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("INSERT INTO multiplier (ilvl, multiplier) VALUES(?, ?)");
            p.setInt(1, ilvl);
    ...
    public static Multiplier getMultiplierForItemlevel(int ilvl)
    {
        ...
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("SELECT * FROM multiplier WHERE ilvl=?");
            p.setInt(1, ilvl);
            ResultSet rs = p.executeQuery();
            while (rs.next()) {
                mp = new Multiplier(rs.getInt("id"), rs.getInt("ilvl"), rs.
                        getDouble("multiplier"));
            }
        } catch (SQLException ex) {
        }
    ...
    public static int updateItem(Items item, String newname, Slots newslot, Type newtype, int newwowid, int newwowidhc, double newprice, double newpricehc, int ilvl, String quality)
    {
        ...
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("UPDATE items SET name=? , wowid_normal=? , wowid_heroic=? , price_normal=? , price_heroic=? , slot=? , type=? , ilvl=? , quality=? WHERE id=?");
        ...
        try {
            if (!itemAlreadyInDatabase(name)) {
                PreparedStatement ps = UnknownEntityDKP.getInstance().getConn().
                        prepareStatement("INSERT INTO items (name, wowid_normal, wowid_heroic, price_normal, price_heroic, slot, type, ilvl, quality) VALUES(?,?,?,?,?,?,?,?,?)");
                ps.setString(1, name);
                ps.setInt(2, wowid);
    ...
    public static int getLootId(int itemid, int charid, double price, Boolean heroic, int raidid)
    {
        ...
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("SELECT * FROM loots WHERE item_id=? AND character_id=? AND price=? AND heroic=? AND raid_id=?");
            p.setInt(1, itemid);
            p.setInt(2, charid);
            p.setDouble(3, price);
            if (heroic) {
    ...
    public static Items getSingleItem(String name)
    {
        ...
        try {
            ...
            while (rs.next()) {
                String temp = rs.getString("type");
                Type tempType = typeFromString(temp);
                item = new Items(rs.getInt("id"), rs.getString("name"), rs.
                        getInt("wowid_normal"), rs.getDouble("price_normal"), rs.
                        getInt("wowid_heroic"), rs.getDouble("price_heroic"), rs.
                        getString("slot"), tempType, rs.getInt("ilvl"), rs.
                        getString("quality"));
    ...
    public static void updateDefaultPrice(String slot, double normalprice)
    {
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("UPDATE default_prices SET price_normal=? WHERE slot=? ");
    ...
    public static ArrayList<RaidItem> getItemsForRaid(int id)
    {
        ...
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("SELECT * FROM loots JOIN items ON loots.item_id = items.id JOIN characters ON loots.character_id = characters.id WHERE loots.raid_id = ?");
            p.setInt(1, id);
            ResultSet rs = p.executeQuery();
            while (rs.next()) {
                RaidItem tmp = new RaidItem(rs.getString("items.name"), rs.
                        getString("characters.name"), rs.getInt("loots.item_id"), rs.
                        getDouble("loots.price"), rs.getBoolean("loots.heroic"), rs.
                        getString("items.quality"));
                items.add(tmp);
            }
        } catch (SQLException ex) {
        }
    ...
    public static Items getItemById(int id)
    {
        ...
        try {
            PreparedStatement ps = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("SELECT DISTINCT * FROM items WHERE id=?");
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                String temp = rs.getString("type");
                Type tempType = typeFromString(temp);
                tmp = new Items(rs.getInt("id"), rs.getString("name"), rs.
                        getInt("wowid_normal"), rs.getDouble("price_normal"), rs.
                        getInt("wowid_heroic"), rs.getDouble("price_heroic"), rs.
                        getString("slot"), tempType, rs.getInt("ilvl"), rs.
                        getString("quality"));
    ...
    private static void updateHeroic(Items item, String pricehc)
    {
        ...
        try {
            PreparedStatement ps = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("UPDATE loots SET price=? WHERE item_id=? AND heroic=1");
    ...
    public static void updateItemLevels(int id, int ilvl, double multiplier)
    {
        ...
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("UPDATE multiplier SET ilvl=? , multiplier=? WHERE id=? ");
            p.setInt(1, ilvl);
            p.setDouble(2, multiplier);
            p.setInt(3, id);
            p.executeUpdate();
    ...
    public static void deleteItemLevelsMultiplier(int id)
    {
        ...
        try {
            PreparedStatement p = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("DELETE FROM multiplier WHERE id=?");
            p.setInt(1, id);
            p.executeUpdate();
        } catch (SQLException ex) {
        }
    ...
    public static double getDefaultPrice(Items item)
    {
        ...
        try {
            PreparedStatement ps = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("SELECT * FROM default_prices WHERE slot=?");
    ...
    private static void updateNormal(Items item, String price)
    {
        ...
        try {
            PreparedStatement ps = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("UPDATE loots SET price=? WHERE item_id=? AND heroic=0");
    ...
    public static void updateItemPrices(int id, BigDecimal formattedprice, BigDecimal formattedpricehc)
    {
        ...
        try {
            PreparedStatement ps = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("UPDATE items SET price_normal=? , price_heroic=? WHERE id=?");
    ...
    public static void updateLootedPrices(int id, BigDecimal formattedprice, BigDecimal formattedpricehc)
    {
        ...
        try {
            PreparedStatement ps = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("UPDATE loots SET price=? WHERE id=? AND HEROIC=0");
            ps.setDouble(1, Double.parseDouble(formattedprice.toString()));
            ps.setInt(2, id);
            ps.executeUpdate();

            ps = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("UPDATE loots SET price=? WHERE id=? AND HEROIC=1");
    ...
    public static List<CharacterItem> getLootForCharacter(String name)
    {
        ...
        try {
            PreparedStatement ps = UnknownEntityDKP.getInstance().getConn().
                    prepareStatement("SELECT loots.heroic,loots.price,items.name,items.id,items.quality FROM loots JOIN items ON items.id=loots.item_id JOIN characters ON loots.character_id=characters.id WHERE characters.name=?");
            ps.setString(1, name);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
